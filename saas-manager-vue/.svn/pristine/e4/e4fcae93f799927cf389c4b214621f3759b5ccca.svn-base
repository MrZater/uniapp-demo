<template>
  <div>
    ...
    <!-- 添加 / 编辑-->
    <el-dialog
      :title="isAdd ? `新增${title} SDK版本` : `编辑${title} SDK版本`"
...
export default {
  ...
  methods: {
    getPlatName(id) {
      let platName = ''
      this.advertSources.forEach((item) => {
        if (item.id == id) {
          platName = item.name
        }
      })
    ...
    async handleadvertDialog() {
      if (this.advertListparams.sourceSdkPackageIds.length == 0) { return this.$message.warning('请选择广告源SDK') }
    ...
    async getPlat() {
      ...
      if (res.code == 200) {
        this.advertSources = res.data ? res.data : []
      }
    },
    async getSdkList() {
      const res = await medproSdkList(this.MedproVparams)
      if (res.code != 200) return this.$message.error('操作成功！')
    ...
    handleSdkVDialog() {
      this.$refs['ruleForm'].validate(async(val) => {
        ...
        if (this.sdkVForm.os == 0) {
          // ios
          if (this.fileListSDK.length == 0) { return this.$message.warning('上传文件为必填项！') }
        } else if (this.sdkVForm.os == 1) {
          if (this.fileListSDK.length == 0 || this.fileListOaid.length == 0) { return this.$message.warning('上传文件为必填项！') }
        }
        if (this.isAdd) {

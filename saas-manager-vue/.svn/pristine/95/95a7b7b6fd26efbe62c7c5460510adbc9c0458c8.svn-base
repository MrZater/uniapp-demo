    ...
    <div class="table">
      ...
      >
        ...
        >
          ...
          />
          <el-table-column
            v-if="searchParams.groupByList.includes('plat_id')"
            align="center"
            :fixed="isFixed"
            prop="platName"
            label="广告源"
...
export default {
    ...
    'searchParams.adminUserIds': {
      async handler(val) {
        if (val.length == 1 && !val[0]) {
          this.searchParams.adminUserIds = []
        }
  ...
  mounted() {
    clearStorage('synthReport', this.storageName)
    this.initDate()
    this.getPosition()
    this.getApp()
    this.getPlace()
  ...
  methods: {
    ...
    async getTableList() {
      ...
      let platarr = []
      if (
        searchParams.groupByList.includes('plat_id') ||
        searchParams.platIdList.length > 0
      ) {
        platarr.push('dau')

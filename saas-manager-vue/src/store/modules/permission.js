...
const actions = {
  generateRoutes({
    ...
    return new Promise(resolve => {
      ...
      if ((user.state.roles[0] === 'ADMIN' || user.state.roles[0] === 'OPERATOR') && user.state.aggregate == 1) {
        asyncRoutes.forEach(item => {
          if (item.path === '/aggregate') {
            item.meta.roles.push(user.state.roles[0])
            item.children.forEach(c => {
              c.meta.roles.push(user.state.roles[0])
            })
          }
        })
      }
      ...
      if (user.state.roles[0] == 'SUPER_ADMIN' ||
        user.state.roles[0] == 'MANAGEMENT' ||
        user.state.roles[0] == 'BUSINESS_DIRECTOR' ||
        user.state.roles[0] == 'OPERATOR_DIRECTOR' ||
        user.state.roles[0] == 'TECHNOLOGY') {
        asyncRoutes.forEach(item => {

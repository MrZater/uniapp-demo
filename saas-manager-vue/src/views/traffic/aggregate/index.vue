  ...
  <div class="aggregate-container">
      ...
      >
        ...
        <div class="filter-container-operate">
          <div class="left">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item>
                  <span
                    class="dropdown_item"
                    @click="handleAdd"
                  ><i class="el-icon-plus" />单个添加</span>
                </el-dropdown-item>
                <el-dropdown-item>
                  <span
                    class="dropdown_item"
                  ...
                  >
                    <i class="el-icon-circle-plus" />批量添加</span>
                </el-dropdown-item>
                ...
                <el-dropdown-item>
                  ...
                  >
                    <i class="el-icon-folder-add" />批量导入</span>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
            <el-button
            ...
            <batchCopyConfig
              :test-id="query.testId"
              :group-id="query.groupId"
              :select-data="selectTableColumn"
        ...
        <div class="chooseBox">
          ...
          >
            <span class="chooseItem">全部已开启</span>
          </el-checkbox>
          <el-checkbox
            v-model="chooseAllS"
            :disabled="Slen == 0"
          ...
          >
            <span class="chooseItem">全部已关闭</span>
          </el-checkbox>
        </div>
        <el-table
          :key="tableKey"
        ...
        >
          ...
          >
            <template slot-scope="{ row }">
              ...
              <div
                v-if="row.type === 'settle'"
                style="color: #1890ff"
              >
                竞价
              </div>
              <div v-if="row.type == 'all'">
                ￥{{ row.placePrice }}
              </div>
            </template>
          </el-table-column>
        ...
        >
          ...
          >
              ...
              <el-row>
                <el-col :lg="1">
                  <el-image
                    ...
                    :src="
                      iconUrl
                        ? this.$store.getters.resourceUrl + iconUrl
                        : '/images/app-none.png'
                    "
                  />
                </el-col>
                ...
                <el-col :lg="5">
                  <div class="ad">
                    ...
                    <div>
                      <span>{{ getPosition(temp.position) }}</span>
                    </div>
                  </div>
                </el-col>
                <el-col :lg="5">
            ...
            <el-form-item
              ...
              prop="platAccountId"
            >
              <el-select v-model="temp.platAccountId">
                <el-option
            ...
            <el-form-item
              v-if="temp.platId == 7 || temp.platId == 11"
              label="AppKey"
              prop="appKey"
            >
              <el-input
                v-model.trim="temp.appKey"
                placeholder="请输入"
              />
            </el-form-item>
            ...
            <el-form-item
              v-if="
                ($store.getters.roles[0] == 'SUPER_ADMIN' &&
                  temp.position == 3 &&
                  temp.isAutoDrawing == '1') ||
                  (accessType == '1' && temp.position == '3')
              "
              label="关闭按钮控制"
              prop="closeButtonType"
            >
              <el-radio
                v-model="temp.closeButtonType"
                :label="'A'"
              >
                一直显示
              </el-radio>
            ...
            <el-form-item
              v-if="
                ((temp.position == 3 && temp.isAutoDrawing == '1') ||
                  (accessType == '1' && temp.position == '3')) &&
                  temp.closeButtonType == 'B' &&
                  $store.getters.roles[0] == 'SUPER_ADMIN'
              "
              label="显示时间"
              prop="countDown"
            >
              ...
              />
              <span style="marginleft: 15px">s后</span>
            </el-form-item>
            <el-form-item
              v-if="
                (temp.position == 3 &&
            ...
            <el-form-item
              v-if="temp.position === 1 && accessType === '1'"
              label="热区"
            >
          ...
          <div
            v-if="isAdvanced"
            class="advanced_form_item"
          >
            <el-form-item
              label-width="100px"
              label="广告源"
            >
              {{ getPlatName(temp.platId) }}
            </el-form-item>
            <el-form-item
              label-width="100px"
              label="代码位"
    ...
    >
      ...
      >
        <el-button
          size="mini"
          @click="closeEndTestDialog"
        >取消</el-button>
        <el-button
          size="mini"
          type="primary"
          @click="handleSubmitEndTest"
        >确定</el-button>
      </span>
    </el-dialog>
    <el-dialog
    ...
    >
      ...
      >
        ...
        <el-form-item v-if="$store.state.user.roles[0] === 'SUPER_ADMIN'">
          ...
          >客户端竞价请求超时时长<tips
            style="display: inline-block"
            content="客户端竞价代码位请求广告超时等待时长"
            :title="''"
            :is-show="true"
          />：</span>
          <el-input-number
            v-model="settingquery.clientTimeout"
            :min="0.1"
            :max="10"
            :step="0.1"
          />&nbsp;s
        ...
        <el-form-item>
          ...
          />：</span>
          <el-switch
            v-model="settingquery.quickRequest"
            active-color="#13ce66"
            inactive-color="#ff4949"
            :active-value="'A'"
            :inactive-value="'S'"
    ...
    >
      ...
      >
        ...
        <el-form-item
          v-if="addAccountQuery.openApiStatus && temp.platId != 17"
          :label="secretLabel"
          prop="secretKey"
        >
          <el-input
...
import { getAppList, platformLinkList } from "@/api/app";
...
export default {
  ...
  computed: {
    ...
    isAutoDrawing() {
      ...
      this.placeOptions.forEach((item) => {
        if (item.id == this.query.placeId) {
          isAutoDrawing =
            item.position == 1 ? true : item.position == 3 ? true : false;
        }
      });
    ...
    isCoopen() {
      ...
      this.placeOptions.forEach((item) => {
        if (item.id == this.query.placeId) {
          isCoopen = item.position == 1 ? true : false;
        }
      });
      return isCoopen;
    },
  ...
  created() {
    clearStorage("aggregateData", this.storageName);
    this.initDate();
  },
  async mounted() {
    // 获取角色
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.id = "";
      if (row.settleType == 1 || row.settleType == 2 || row.settleType == 3) {
        row.isBidding = true;
      } else {
        row.isBidding = false;
      }
    ...
    async handleSubmitEndTest() {
      this.$refs.endtestdialog.validate(async (val) => {
        ...
        let res = await getPlaceGroup(this.query.placeId);
        this.config = res.data;
        this.config.groups.forEach(async (item) => {
          if (item.id === this.query.groupId) {
            this.group = JSON.parse(JSON.stringify(item));
          }
        });
        this.abTestList = [];
        this.query.testId = "default";
          ...
          if (
            ...
            this.config.groups[i].abTests.forEach((child, c) => {
              if (child.testId === this.query.testId) {
                ...
                clientTimeout =
                  +this.config.groups[i].abTests[c].clientTimeout / 1000;
                cacheTime = +this.config.groups[i].abTests[c].cacheTime;
                cacheSts = this.config.groups[i].abTests[c].cacheSts;
                quickRequest = this.config.groups[i].abTests[c].quickRequest;
              }
            });
          } else {
            ...
            clientTimeout = +this.config.groups[i].clientTimeout / 1000;
            cacheTime = +this.config.groups[i].cacheTime;
            cacheSts = this.config.groups[i].cacheSts;
            quickRequest = this.config.groups[i].quickRequest;
          }
        }
      });
      this.settingquery = {
    ...
    async handleSetting() {
      ...
      let clientTimeout = +this.settingquery.clientTimeout * 1000;

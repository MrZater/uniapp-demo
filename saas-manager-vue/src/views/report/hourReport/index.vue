    ...
    <div class="search">
      ...
      >
        ...
        >
          <CustSelectOne
            style="width: 95%"
            :model="reportParams.adminUserIds[0]"
            :all-data="adminList"
            :width="285"
            word="账号"
            :clear="true"
        ...
        >
          ...
          <CustSelectApp
            ...
            :word="'广告位'"
            :width="450"
            :model="reportParams.placeIdList"
...
export default {
  name: "HourReport",
  components: { Empty, tips, CustSelectApp, CustSelectOne },
  data() {
    return {
      myEcharts: null,
      iscreat: true,
      ...
      adverStyleVal: "",
      systemPlatform: [],
      showFlow: false, // 广告源及代码位有值时隐藏流量请求、流量填充率
    };
  },
  computed: {
    title() {
  ...
  mounted() {
    ...
    if (
      ...
      this.reportParams.appIdList.push(this.$route.params.crow.appId);
    }
    if (
      this.$route.params &&
      this.$route.params.crow &&
      ...
      this.reportParams.placeIdList.push(this.$route.params.crow.placeId);
    }
    if (this.$route.params && this.$route.params.row) {
      this.reportParams.dateEnd = this.$route.params.row.oldDate;
      this.reportParams.dateStart = this.$route.params.row.newDate;
      this.time[0] = this.$route.params.row.oldDate;
      this.time[1] = this.$route.params.row.newDate;
    }
    this.getHoursReport();
  ...
  methods: {
    ...
    getDate() {
      ...
      // 13天前
      let yesterday = toDate(Date.now() - 24 * 60 * 60 * 1000);
      this.time = [yesterday, today];
    ...
    getHoursReport() {
      if (
        this.reportParams.adminUserIds.length == 1 &&
        !this.reportParams.adminUserIds[0]
      ) {
      ...
      hoursReport(this.reportParams).then((res) => {
        ...
        if (
          this.reportParams.sourceIdList.length > 0 ||
          this.reportParams.platIdList.length > 0
        ) {
          this.showFlow = false;
        } else {
          this.showFlow = true;
    ...
    handleSelectAdmin(val) {
      this.reportParams.adminUserIds = [val];
    },
    ...
    selectappListhandleCode(val) {
      this.reportParams.sourceIdList = val;
    },
    ...
    getHoursEcharts() {
      ...
      hoursEcharts(this.reportParams).then((res) => {
        ...
        if (this.dateArr.length == 0) {
          this.firstEcjartsList = [];
          this.secondEcjartsList = [];
        }
        this.changeEcharts();
      });
    },
    ...
    changeEcharts() {
      ...
      if (this.firstEcjartsList.length == 0) {
        this.option.legend = {
          data: [],
        };
        this.option.series[0] = {
      ...
      if (this.secondEcjartsList.length == 0) {
        this.option.legend.data = this.option.legend.data.splice(1, 0);
        this.option.series[1] = {
          name: "",
          type: "line",
          smooth: true,
    ...
    exportHourReport() {
      if (this.hoursTableData.length == 0) return;
...
::v-deep.tab-container .search .cust-select-container .osicon {
  display: none;
}
</style>

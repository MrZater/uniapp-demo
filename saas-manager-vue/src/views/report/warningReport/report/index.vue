        ...
        >
          <template slot-scope="{ row }">
            ...
            >
              ...
              >
                <template slot-scope="{ row }">
                  <div v-if="row.placeName">
                    {{ row.placeName }}
                  </div>
                  <div v-if="row.placeId">
                    ID：{{ row.placeId }}
                  </div>
                  <span v-if="!row.placeName">-</span>
                </template>
              </el-table-column>
              <el-table-column
                align="center"
              ...
              >
                <template slot-scope="scope">
                  <el-link
                    type="success"
                    @click="handleDetails(scope.row, row)"
                  >
                    查看更多数据
                  </el-link>
                </template>
...
export default {
  ...
  methods: {
    ...
    initDate() {
      ...
      let time2 = new Date().getTime() - 1000 * 60 * 60 * 24 * 7
    ...
    toogleExpand(row) {
      ...
      if (this.expandId != row.id) {
        $table.toggleRowExpansion(row)
        this.expandId = row.id
      } else if (this.expandId == row.id) {
        this.expandId = ''
      }
    },
...
::v-deep.table .reportItem {
  background-color: rgb(245, 255, 256);
}
</style>

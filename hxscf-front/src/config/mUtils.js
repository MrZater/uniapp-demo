...
function data2treeDG(datas, dataArray) {
    ...
    return dataArray.sort((a, b) => a.orders - b.orders);
}
// 处理资源树
...
function authorityTree(datas = []) {
    ...
    datas.forEach(function (data) {
        let pid = data.resPcode;
        // 如果当前节点存在 resPcode ，同时数据中又没有 resCode == resPcode 的数据，则将当前节点处理为根节点
        var hasParentData = datas.some(el => el.resCode === data.resPcode);
        if (!pid || !hasParentData) {
            let objTemp = {
...
function organizationTree(datas = []) {
    var dataArray = [];
    datas.forEach(function (data) {
        let pid = data.orgPcode;
        // 如果当前节点存在 orgPcode ，同时数据中又没有 orgCode == orgPcode 的数据，则将当前节点处理为根节点
        var hasParentData = datas.some(el => el.orgCode === data.orgPcode);
        if (!pid || !hasParentData) {
            let objTemp = {
                label: data.orgName,
                id: data.orgCode,
                ...
                orgNo: data.orgNo,
                // ------------------------------------------增
                orgType: data.orgType, //机构类型
                // orgCode: data.orgNo, //组织代码
                orgOrder: data.orgOrder, //排序
                createTime: data.createTime, //创建时间
                // saveFlag: data.saveFlag,
                orgPcode: data.orgPcode, // 上级机构
                isDel: data.isDel //状态
            };
            dataArray.push(objTemp);
        }
...
export const cityFN = dataArray => {
    let arr = [];

    dataArray.forEach((item) => {
        let newObj = {};
        newObj.label = item.areaName;
        newObj.value = item.areaCode;
        newObj.Pcode = item.areaPcode;
        arr.push(newObj)
    });
    return DefinitionTree(arr)
};

function DefinitionTree(datas) {
    var dataArray = []; // 带有pcode的根数据; 
    datas.forEach(function (data) {
        let pid = data.Pcode;
        if (pid == '0') {
            dataArray.push(data);
    ...
    return DefinitionDG(datas, dataArray);
}

function DefinitionDG(datas, dataArray) {
    let isChildCode = []
    for (var j = 0; j < dataArray.length; j++) {
        let dataArrayIndex = dataArray[j];
        let childrenArray = [];
        let code = dataArrayIndex.value;
        for (let i = datas.length - 1; i >= 0; i--) {
            let data = datas[i];
            let pid = data.Pcode;
            if (pid == code) {
                ...
                isChildCode.push(data.value);
            }
        }
        childrenArray.length > 0 ? (dataArrayIndex.children = childrenArray) : null;
        if (childrenArray.length > 0) {
            //有儿子节点则递归
            DefinitionDG(datas, childrenArray);
        }
    }
    return dataArray.filter(item => !isChildCode.includes(item.value));
}

// 获取上级机构
export const dealOrganizationPCode = dataArry => {
    let dataArray = [];
...
// 获取目录权限
// export const dealDicPermisson = dataArry => {
//     let dataArray = [];
//     // debugger
//     dataArry.forEach(function (data) {
//         let pid = data.resPcode;
...
// 获取目录权限
export const dealDicPermisson = dataArry => {
    return authorityTree1(dataArry)
}


function authorityTree1(datas = []) {
    var dataArray = [];
    datas.forEach(function (data) {
        let pid = data.resPcode;
        if (!pid) {
    ...
    return resouceDataTreeDG1(datas, dataArray);
}

function resouceDataTreeDG1(datas, dataArray) {
    let isChildCode = []
    for (var j = 0; j < dataArray.length; j++) {
        let dataArrayIndex = dataArray[j];
        let childrenArray = [];

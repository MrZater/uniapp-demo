<template>
    <B-dialog :title="handleType === 'add' ? '新增' :handleType === 'reSubmit'?'重新提交': '提交审核'" :visible="dialogVisible" width="700px" @close="handleCancel">
        <el-form :model="dialogForm" :rules="rules" :inline="true" ref="forms" size="small" v-loading="loading">
            ...
            <el-table stripe :data="dialogForm.riskRatioList" class="br-table" border style="width: 100%">
                <el-table-column label="客户信用等级" prop="riskLevel" width="150"></el-table-column>
                <el-table-column label="通用客户风险系数" prop="riskRatio" :render-header="renderHeader">
                    <template slot-scope="scope">
                        <el-form-item :prop="'riskRatioList.' + scope.$index + '.riskRatio' " class="br-form-item-label" :rules="rules.riskRatio">
        ...
        <template slot="footer">
            <el-button class="br-btn-large" @click="handleCancel" :disabled="loading">取 消</el-button>
...
export default class addOrUpdate extends Vue {
    ...
    @customerRisk.Action queryAddOrUpdate;
    @customerRisk.Action queryRecordList;
    ...
    renderHeader(h, { column }) {
        return h("div", [
            ...
            h("span", { style: "margin-left:4px;" }, column.label),
        ]);
    }
    ...
    initData(row, type) {
        ...
        if (type === "add") {
            this.$set(this.dialogForm, "riskRatioList", row);
        } else {
            this.dialogForm.riskId = row.riskId;
            if (type === "reSubmit") {
                // 获取日志

<template>
    <div v-loading="loading">
        <el-form :inline="true" :model="queryForm" ref="form" size="small">
            <el-form-item class="br-form-item-label" label="数据年份" prop="indexDate">
                <span>{{queryForm.indexDate}}</span>
            </el-form-item>
        </el-form>
...
export default class creditInvestigation extends Vue {
    @tradeInfoManage.Action queryTradeDetailBase; //查看行业基础数据
    ...
    mounted() {
        this.loading = true;
        this.queryTradeDetailBase({ keyNo: this.$route.query.keyNo })
            .then((res) => {
